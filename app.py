...
try:
    # Load ANTHROPIC_API_KEY / RAG_LLM_MODEL from a local .env file if present.
    # Optional: if python-dotenv isn't installed, environment variables still work.
...
if "claude_generator" not in st.session_state:
    # Create the Claude client once per session, only if an API key is present.
    st.session_state.claude_generator = (
        ClaudeAnswerGenerator() if ClaudeAnswerGenerator.is_available() else None
    )
...
# Answer generation engine: Claude (grounded LLM) when available, otherwise the
# rule-based extractive fallback so the app still runs without an API key.
st.sidebar.markdown('<div class="section-header">🧠 Answer Engine</div>', unsafe_allow_html=True)
claude_generator = st.session_state.claude_generator
if claude_generator is not None:
    use_llm = st.sidebar.toggle("Use Claude for answers", value=True)
    st.sidebar.caption(f"Model: `{claude_generator.model}`")
else:
    use_llm = False
    st.sidebar.caption(
        "Set `ANTHROPIC_API_KEY` to enable Claude-generated answers. "
        "Using the extractive fallback for now."
    )
    ...
    with col_submit:
        if st.button("🔍 Search & Generate Answer", use_container_width=True):
            if query.strip():
                ...
                # Display the retrieved context first so the answer can be
                ...
                if use_llm:
                    # Grounded generation with Claude, streamed as it is produced.
                    try:
                        st.write_stream(
                            claude_generator.stream_answer(query, results)
                        )
                        st.caption(
                            f"Generated by Claude (`{claude_generator.model}`), "
                            "grounded in the retrieved context above."
                        )
                    except Exception as error:
                        st.warning(
                            f"Claude generation failed ({error}). "
                            "Falling back to the extractive answer."
                        )
                        st.markdown(AnswerGenerator.generate_answer(query, results))
                        st.caption("Extractive fallback (no LLM).")
                else:
                    # Rule-based extractive answer (no API key required).
                    st.markdown(AnswerGenerator.generate_answer(query, results))
                    st.caption(
                        "Extractive fallback. Set `ANTHROPIC_API_KEY` to enable "
...
st.markdown("""
    <div style="text-align: center; color: gray; font-size: 0.9em; margin-top: 20px;">
        <p>RAG Data Assistant | Streamlit · Sentence-Transformers · FAISS · Claude</p>
        <p>Built as a portfolio project | Runs locally; Claude answers are optional</p>
    </div>
""", unsafe_allow_html=True)
